...
public final class LinkedInMarketingController {
   ...
   * 
   * @return All Ad Accounts that an authenticated user has access to can be
   *         retrieved with the following endpoint.
   */
  ...
  public String Get_user_org_access() {
    try {
      setUserAgentHeader();
      String response = lmsTemplate
          .exchange(LI_FIND_USER_ROLES_ENDPOINT + token, HttpMethod.GET, new HttpEntity(header), String.class)
          .getBody();
   ...
   * public String Fetch_ad_account(@RequestParam(name = "account", required =
   * false) final String account, final HttpSession session) {
   * if (account == null) {
   ...
   * String response =
   * lmsTemplate.getForObject("https://api.linkedin.com/v2/adAccountsV2/" +
   * account + "?oauth2_access_token=" + token, String.class);
   * return response;
   * } catch (HttpStatusCodeException e) {
   * return e.getResponseBodyAsString();
   * }
